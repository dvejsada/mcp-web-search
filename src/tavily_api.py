...
async def tavily_search(query, lang):

    tavily_client = AsyncTavilyClient(api_key=TAVILY_API_KEY)

    # Executing a context search query
    try:
        context = await tavily_client.search(query=query, include_answer=True, max_results=RESULT_NUMBER)
...
async def tavily_extract(urls: list):
    ...
    # Executing the extract request
    response = await tavily_client.extract(urls=urls, include_images=False)
